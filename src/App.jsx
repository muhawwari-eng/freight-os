...
export default function App() {
  ...
  const globalSearchResults = useMemo(() => {
    ...
    return shipments.flatMap((shipment) => {
      ...
      getFinancialInvoices(shipment).forEach((invoice) => {
        if ([invoice.invoiceNo, invoice.party, invoice.category, invoice.note, invoice.attachmentName, invoice.adjustmentKind].some((value) => String(value || "").toLowerCase().includes(term))) {
          matches.push({ type: "Invoice", label: `${invoice.invoiceNo || "Invoice"} | ${invoice.party || ""} | ${invoice.category || ""}`, shipment });
        }
      });
      getPayments(shipment).forEach((payment) => {
        if ([payment.id, payment.purchaseType, payment.company, payment.note, payment.paidDate].some((value) => String(value || "").toLowerCase().includes(term))) {
          matches.push({ type: "Payment", label: `${payment.purchaseType || "Payment"} | ${payment.company || ""} | ${money(payment.amount, payment.currency || "USD")}`, shipment });
        }
      });
  ...
  const actionCenter = useMemo(() => {
    ...
    shipments.forEach((shipment) => {
      ...
      if (canSeeFinance) {
        ledger.rows
          ...
          .forEach((row) => {
            items.push({
              id: `approval-${shipment.id}-${row.id}`,
              severity: "warning",
              type: "Approval",
              title: "Invoice awaiting approval",
              detail: `${row.invoiceNo || "Invoice"} for ${row.party || "Party"} is pending approval for ${money(row.totalUsd)}.`,
        ...
        ledger.rows
          .filter((row) => !row.autoGenerated && !row.attachmentDataUrl && Math.abs(row.totalUsd) >= 500)
          .forEach((row) => {
            items.push({
              id: `attachment-${shipment.id}-${row.id}`,
              severity: "info",
              type: "Document Control",
              title: "Invoice attachment missing",
              detail: `${row.invoiceNo || "Invoice"} has no attached supplier/customer document.`,
              meta: `${shipment.id} | ${route}`,
              daysLeft: 5,
              shipment,
            });
        ...
        ledger.saleRows
          ...
          .forEach((row) => {
            ...
            items.push({
              ...
              detail: `${row.party || shipment.customer || "Customer"} owes ${money(row.remainingUsd)} for ${row.invoiceNo || shipment.id}. Due ${row.dueDate}.`,
              meta: `${shipment.id} | ${route}`,
              daysLeft,
              shipment,
            });
        ...
        ledger.saleRows.forEach((row) => {
          ...
          if (!limit) return;
          const customerOpen = shipments.reduce((sum, item) => {
            const itemLedger = getShipmentFinancialLedger(item, activeFxRate);
            return sum + itemLedger.saleRows
              .filter((invoice) => (invoice.party || item.customer) === customer.name)
              .reduce((invoiceSum, invoice) => invoiceSum + invoice.remainingUsd, 0);
          }, 0);
          if (customerOpen > limit) {
            items.push({
              id: `credit-${customer.name}-${shipment.id}-${row.id}`,
              severity: "high",
              type: "Credit Limit",
              title: "Customer credit limit exceeded",
              detail: `${customer.name} open balance ${money(customerOpen)} is above limit ${money(limit)}.`,
              meta: `${shipment.id} | ${route}`,
              daysLeft: -1,
              shipment,
            });
    ...
    return items
      ...
      .slice(0, 18);
  }, [shipments, activeFxRate, canSeeFinance, customers]);
...
function addShipmentFromForm(e) {
  ...
  function saveFinancialInvoice(shipmentId, invoiceForm, editingInvoiceId, calculatedInvoiceId) {
    ...
    setShipments((previous) => previous.map((shipment) => {
      ...
      if (shipment.financialClosed) {
        alert("This financial file is closed. Reopen it before changing invoices.");
        return shipment;
      }
  ...
  function deleteFinancialInvoice(shipmentId, invoiceId) {
    ...
    setShipments((previous) => previous.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      if (shipment.financialClosed) {
        alert("This financial file is closed. Reopen it before deleting invoices.");
        return shipment;
      }
  ...
  function approveFinancialInvoice(shipmentId, invoiceId) {
    if (!canManagePayments) return;
    let updatedSelected = null;
    setShipments((previous) => previous.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      const invoice = getFinancialInvoices(shipment).find((row) => row.id === invoiceId);
      const updated = normalizeShipment(withTimeline({
        ...shipment,
        financialInvoices: getFinancialInvoices(shipment).map((row) => (
  ...
  function toggleFinancialClosed(shipmentId, reason = "") {
    if (!canManagePayments) return;
    const note = reason.trim();
    if (!note) {
      alert("Please enter a reason before closing or reopening this financial file.");
      return;
    }
    let updatedSelected = null;
    setShipments((previous) => previous.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      const nextClosed = !shipment.financialClosed;
      const updated = normalizeShipment(withTimeline({
        ...shipment,
        financialClosed: nextClosed,
        financialClosedAt: nextClosed ? new Date().toISOString() : "",
        financialClosedBy: nextClosed ? user?.email || "unknown" : "",
        financialReopenReason: nextClosed ? shipment.financialReopenReason || "" : note,
        financialCloseReason: nextClosed ? note : shipment.financialCloseReason || "",
      }, createTimelineEvent("Finance", nextClosed ? "Financial file closed" : "Financial file reopened", note)));
      if (selectedShipment?.id === shipmentId) updatedSelected = updated;
      return updated;
    }));
    if (updatedSelected) setSelectedShipment(updatedSelected);
  ...
  function addInvoicePayment(shipmentId, invoice, invoicePaymentForm) {
    ...
    setShipments((previous) => previous.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      if (shipment.financialClosed) {
        alert("This financial file is closed. Reopen it before recording payments.");
        return shipment;
      }
      const updated = normalizeShipment(withTimeline(
  ...
  function addAllocatedInvoicePayment(shipmentId, invoicePaymentForm, allocations) {
    if (!canManagePayments) {
      alert("Only admin can add payments.");
      return;
    }
    const validAllocations = allocations
      .map((allocation) => ({ invoiceId: allocation.invoiceId, amountUsd: Number(allocation.amountUsd || 0) }))
      .filter((allocation) => allocation.invoiceId && allocation.amountUsd > 0);
    if (!validAllocations.length) {
      alert("Please allocate at least one amount to an invoice.");
      return;
    }
    ...
    setShipments((previous) => previous.map((shipment) => {
      ...
      const ledger = getShipmentFinancialLedger(shipment, activeFxRate);
      const selectedInvoices = validAllocations.map((allocation) => ledger.rows.find((row) => row.id === allocation.invoiceId)).filter(Boolean);
      const paymentType = selectedInvoices.length === 1 ? getInvoicePaymentType(selectedInvoices[0]) : "Mixed Allocation";
      const totalUsd = validAllocations.reduce((sum, allocation) => sum + allocation.amountUsd, 0);
      const currency = invoicePaymentForm.currency || "USD";
      const fxRate = Number(invoicePaymentForm.fxRate || shipment.fx || activeFxRate || 1);
      const amount = currency === "USD" ? totalUsd : totalUsd * fxRate;
      const newPayment = {
        id: `PAY-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        purchaseType: paymentType,
        company: invoicePaymentForm.company || selectedInvoices.map((invoice) => invoice.party).filter(Boolean).join(", ") || "Multiple parties",
        amount,
        taxRate: Number(invoicePaymentForm.taxRate || 0),
        currency,
        fxRate,
      ...
      const updated = normalizeShipment(withTimeline(
        { ...shipment, payments: [newPayment, ...getPayments(shipment)] },
        createTimelineEvent("Payment", "Allocated payment recorded", `${validAllocations.length} invoice(s) | ${money(totalUsd)}`)
      ));
      if (selectedShipment?.id === shipmentId) updatedSelected = updated;
      return updated;
    }));
  ...
  function assignInvoicePayment(shipmentId, paymentId, invoiceId) {
    ...
    setShipments((previous) => previous.map((shipment) => {
      if (shipment.id !== shipmentId) return shipment;
      if (shipment.financialClosed) {
        alert("This financial file is closed. Reopen it before allocating payments.");
        return shipment;
      }
      const updated = normalizeShipment(withTimeline({
        ...shipment,
