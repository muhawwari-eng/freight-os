...
const EMAILJS_SERVICE_ID = "service_zty1pxo";
const EMAILJS_TEMPLATE_ID = "template_ivz6t4r";
const EMAILJS_PUBLIC_KEY = "pCTprIZ8Br_3xrxlx";
const OPERATION_EMAIL = "m.hawari@fsclojistik.com";
...
function generateReceiptPdf(shipment, payment, exchangeRate) {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
...
export default function App() {
  ...
  useEffect(() => {
    if (!user?.id || !onlineDataLoaded || !canEditOperation || reminderAutoCheckRef.current) return;
    reminderAutoCheckRef.current = true;
    checkAndSendReminders("auto");
  }, [user?.id, onlineDataLoaded, canEditOperation]);
  ...
  function buildReminderCandidates() {
    ...
    return shipments.flatMap((shipment) => {
      ...
      return getReminderEventsForShipment(shipment)
        ...
        .filter((event) => sent[event.key] !== toDateKey(event.eventDate))
  ...
  async function sendReminderForCandidate(candidate) {
    ...
    const recipients = [
      { type: "operation", email: OPERATION_EMAIL, name: "Operation Team" },
    ];
  ...
  async function checkAndSendReminders(source = "manual") {
    ...
    if (!candidates.length) {
      if (source === "manual") {
        pushNotification({ type: "info", title: "No reminders to send", message: "No Cut-Off, Departure, or Arrival reminders due tomorrow." });
      }
      return;
    }
